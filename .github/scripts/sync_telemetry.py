...
def fetch_live_stats(username, token):
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            # 1. Basic Stats
            total_contributions = cal.get("totalContributions", 325)
        ...
        print("Error fetching live GitHub stats via GraphQL:", e)
        return None
...
def fetch_all_public_commits(username="Arnim-Zola", token=None):
    url = f"https://api.github.com/users/{username}/repos?per_page=100"
    headers = {"User-Agent": "Arnim-Zola-Telemetry-Bot"}
    if token:
    ...
    try:
        ...
        for r in repos_data:
            ...
            page = 1
            while True:
                c_url = f"https://api.github.com/repos/{username}/{rname}/commits?per_page=100&page={page}"
                c_req = urllib.request.Request(c_url, headers=headers)
                with urllib.request.urlopen(c_req, timeout=15) as c_resp:
                    commits = json.loads(c_resp.read().decode())
                    if not commits:
                        break
        ...
        print("Error fetching all public commits:", e)
        return 328
...
def sync_telemetry():
    # Detect repository root dynamically (works both locally and in GitHub Actions runner)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    repo_root = os.path.dirname(os.path.dirname(script_dir)) if "scripts" in script_dir else os.path.dirname(script_dir)
    ...
    if live_stats:
        print("Using freshly fetched live GitHub GraphQL telemetry stats!")
        total_contributions = live_stats["total_contributions"]
        total_commits = fetch_all_public_commits(username, token) or live_stats["total_commits"]
        ...
        print("Using public API & calendar scraping.")
        public_weeks, real_total = fetch_public_contribution_calendar(username)
        total_contributions = real_total if real_total > 0 else 325
        weeks = public_weeks

        all_days = [d for w in (weeks or []) for d in w.get("contributionDays", [])]
        longest_streak = 0
        longest_start = None
        longest_end = None
        temp_streak = 0
        ...
        total_commits = fetch_all_public_commits(username, token)
        total_prs = 3
        total_issues = 1
        total_stars = 5
        languages = [
  ...
  <g transform="translate({card_x}, 450)" filter="url(#cardShadow)">
    ...
    for fname in ["telemetry-cosmos-card.svg", "telemetry-cosmos-card-v1.svg", "telemetry-cosmos-card-v2.svg", "telemetry-cosmos-card-v3.svg", "telemetry-cosmos-card-v4.svg", "telemetry-cosmos-card-v5.svg", "telemetry-cosmos-card-v6.svg", "telemetry-cosmos-card-v7.svg", "telemetry-cosmos-card-v8.svg", "telemetry-cosmos-card-v9.svg", "telemetry-cosmos-card-v10.svg", "telemetry-cosmos-card-v11.svg"]:
        out_path = os.path.join(assets_dir, fname)
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(svg_content)
    ET.fromstring(svg_content)
    print("Telemetry successfully generated & validated for assets/telemetry-cosmos-card*.svg")
